    ...
    try:
        ...
        if db_name not in client.list_database_names():
            if create_if_not_exists:
                # If admin credentials were provided, allow "creation" of the competition DB.
                db = client[db_name]
                # Optionally, you could also create the collections explicitly:
                # db.create_collection("entries")
                # db.create_collection("princess")
                return db
            else:
                return None
        else:
